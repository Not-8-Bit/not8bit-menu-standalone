if NM.VersionCheck then
    local function VersionLog(_type, log)
        local color = _type == 'success' and '^2' or '^1'
        print(('^8[not8bit-menu]%s %s^6'):format(color, log))
    end

    local function CheckMenuVersion()
        PerformHttpRequest('https://raw.githubusercontent.com/Not-8-Bit/not8bit-menu/main/version.txt', function(err, text, headers)
            ...
            if text:gsub("%s+", "") == currentVersion:gsub("%s+", "") then
                VersionLog('success', ('You are running the latest version %s of not8bit-menu by lllHolidaylll.'):format(currentVersion))
            else
